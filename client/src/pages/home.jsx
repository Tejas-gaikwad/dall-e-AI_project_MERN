...
const Home = () => {
  ...
  const [setSearchTimeout, setsetSearchTimeOut] = useState(null)
   useEffect(() => {
    const fetchPosts = async () => {
      setLoading(true);
      try {
        var response = await fetch('http://localhost:8080/api/v1/post', {
        method:'GET',
        headers: {
      ...
      <div className='mt-10'>
        {
          ...
          <div>
            {searchText && 
              <h2 className='font-medium text-[#666e75] text-xl mb-3'>Showing resuls for  <span className=' text-[#222328]'>{  searchText}</span>
              </h2>
            }
